...
def main(config):
    ...
    for epoch in range(epochs):
        ...
        for input, target in train_bar:
            ...
            if config.aug == 'mixup':
                inputs, labels_a, labels_b, lam = mixup_data(input, target)
                ...
                inputs, labels_a, labels_b, lam = cutmix((input, target))
                inputs, labels_a, labels_b = map(Variable, (inputs, labels_a, labels_b))
                output = net(inputs)
                ...
                predict = torch.max(output, dim=1)[1]
                train_acc += torch.eq(predict, target).sum().item()
        ...
        val_acc = acc / val_num
        mean_loss = eval_loss / test_steps
